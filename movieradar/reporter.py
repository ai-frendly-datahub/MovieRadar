...
def generate_report(
    *,
    ...
    if quality_report:
        _inject_movie_quality_panel(report_path, quality_report)
    return report_path
...
def generate_index_html(
    ...
    radar_name = "Movie Radar"
    return _core_generate_index_html(report_dir, radar_name)


def _inject_movie_quality_panel(report_path: Path, quality_report: Mapping[str, Any]) -> None:
    if not report_path.exists():
        return
    html = report_path.read_text(encoding="utf-8")
    panel = _render_movie_quality_panel(quality_report)
    marker = "</body>"
    if marker in html:
...
def _render_movie_quality_panel(quality_report: Mapping[str, Any]) -> str:
    ...
    cards = [
        ("Movie signals", summary.get("movie_signal_event_count", 0)),
        ("Reservation", summary.get("reservation_ranking_events", 0)),
        ("Box office", summary.get("box_office_events", 0)),
        ("OTT ranking", summary.get("ott_ranking_events", 0)),
        ("Release calendar", summary.get("release_calendar_events", 0)),
        ("Movie IDs", summary.get("movie_id_present_count", 0)),
        ("Required gaps", summary.get("event_required_field_gap_count", 0)),
        ("Review items", summary.get("daily_review_item_count", 0)),
    ]
    cards_html = "\n".join(
        "<div class=\"movie-quality-card\">"
        f"<span>{escape(label)}</span><strong>{escape(str(value))}</strong>"
        "</div>"
        for label, value in cards
    )
    return f"""
<section id="movie-quality" class="movie-quality-panel">
  <style>
    .movie-quality-panel {{ margin: 32px auto; max-width: 1180px; padding: 24px; border: 1px solid #d8dee4; border-radius: 8px; background: #fff; color: #24292f; }}
    .movie-quality-panel h2 {{ margin: 0 0 8px; font-size: 1.35rem; }}
    .movie-quality-grid {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 10px; margin: 16px 0 22px; }}
    .movie-quality-card {{ border: 1px solid #d8dee4; border-radius: 8px; padding: 10px 12px; background: #f6f8fa; }}
    .movie-quality-card span {{ display: block; font-size: .82rem; color: #57606a; }}
    .movie-quality-card strong {{ display: block; margin-top: 4px; font-size: 1.2rem; }}
    .movie-quality-table {{ width: 100%; border-collapse: collapse; margin-top: 12px; font-size: .9rem; }}
    .movie-quality-table th, .movie-quality-table td {{ border-top: 1px solid #d8dee4; padding: 8px; text-align: left; vertical-align: top; }}
    .movie-quality-review {{ margin: 12px 0 0; padding-left: 18px; }}
  </style>
  <h2>Movie Quality</h2>
  <p>Box-office, reservation, OTT ranking, and release-calendar evidence is tracked separately from entertainment coverage.</p>
  <div class="movie-quality-grid">
    {cards_html}
  </div>
...
def _render_quality_events(events: list[Mapping[str, Any]]) -> str:
    if not events:
        return "<p>No movie quality events were observed in this report window.</p>"
    rows = []
    ...
    return (
        ...
        "<table class=\"movie-quality-table\"><thead><tr>"
...
def _render_quality_review(review_items: list[Mapping[str, Any]]) -> str:
    ...
    return "<h3>Daily Review</h3><ul class=\"movie-quality-review\">" + "\n".join(items) + "</ul>"
